...
def scrapeEachPost(url, thumbnail):
    postFoundInDB = Album.objects(url=url, source=source)
    if ~(len(postFoundInDB) == 0):
        ...
        # post = Album(title=title, source=source, url=url,images=images,thumbnail=thumbnail )
        # post.save()


def scrapeMainPage():
    html = BeautifulSoup(requests.get(
        mainUrl, verify=False).text, 'html.parser')

    postHtmlList = html.find(class_='index_listc').find(
        class_='pos_6_1').find('ul').find_all('li')

    for postHtml in postHtmlList:
        postUrl = mainUrl + postHtml.find('a').get('href').split('/')[2]

        thumbnail = postHtml.find('a').find(
            'img').get('src').replace('//', 'https://')
